        ...
        <div className="flex items-center gap-4">
           ...
           className="cursor-pointer">{ isOwner ? 'Dashboard' : 'List cars'}</button> 
            <button onClick={()=> {user ? logout(): setShowLogin(true)}} 
             className="px-8 py-2 bg-[var(--color-primary)] text-white rounded-lg active:scale-95 transition bg--500 rounded text-white shadow-lg shadow-blue-500/30 text-sm font-medium">{user ? 'Logout' : 'Login'}</button>
        </div>
      </div>
